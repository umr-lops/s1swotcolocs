...
def process_nc_to_stac(nc_path, output_dir, config, extension_url,counters,overwrite):
    ...
    try:
        ...
        for i in range(num_matches):
            ...
            swot_formated_date,year,month,day = get_swot_date_info(SWOT_start_piece=ds.SWOT_start_time_slice.values)
...
def main():
    ...
    group.add_argument("--input-nc", type=str,help='individual full path of SWOT KaRin matchups .nc')
    group.add_argument("--input-list", type=str,help='listing of path of SWOT KaRin matchups .nc')
    parser.add_argument("--config", type=str, required=True, help="Path to localconfig.yml")
    parser.add_argument("--out-dir", type=str, default=".",required=False,help='where to store STAC files [optional, default is .]')
    ...
    for f in tqdm(files, desc="Converting", disable=args.verbose):
        ...
        tmp_list_outstac,counters = process_nc_to_stac(f, args.out_dir, config, 
                    DEFAULT_EXTENSION_SCHEMA,counters=counters,overwrite=args.overwrite)
        final_listing_stac_generated += tmp_list_outstac
    if len(final_listing_stac_generated)>0:
        logging.info('example of output file STAC generated : %s',final_listing_stac_generated[-1])
    logging.info('counters: %s',counters)
    logging.info('Number of STAC files generated: %s',len(final_listing_stac_generated))
