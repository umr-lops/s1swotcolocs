...
def main():
    parser = argparse.ArgumentParser(description="Téléchargeur SWOT L2 par plage de dates")
    ...
    parser.add_argument("--host", default="ftp-access.aviso.altimetry.fr", help="Hôte SFTP [optional défaut: ftp-access.aviso.altimetry.fr]")
    parser.add_argument("--port", type=int, default=2221, help="Port SFTP [optional default: 2221]")
    parser.add_argument("--productID",choices=['PID0','PGD0'],default='PID0',help="ID du produit SWOT L2 (PID0 ou PGD0) [optionnel défaut: PID0]")
    parser.add_argument('--verbose', action='store_true', help="Activer le mode verbeux")
    parser.add_argument('--dry-run', action='store_true', help="Exécuter en mode test sans téléchargement"  )
    ...
    try:
        ...
        for cci,cycle_dir in enumerate(cycles):
            ...
            logger.info(f"Analyse du dossier {cycle_dir} number {cci+1} sur {len(cycles)}")
                    ...
                    if file_date and start_dt <= file_date <= end_dt:
                        ...
                        try:
                            if args.dry_run:
                                ...
                                download_with_progress(sftp, remote_file_path, local_file_path, filename)
                                stats["downloaded"] += 1
