...
@patch("s1swotcolocs.download_l2_swot_karin_lr_ssh_windwave_cnes.argparse.ArgumentParser.parse_args")
@patch("s1swotcolocs.download_l2_swot_karin_lr_ssh_windwave_cnes.download_with_progress")
def test_main_execution_logic(mock_download, mock_args, mock_exists_patch, mock_makedirs, mock_ssh_class):
    # 1. Setup Mock Arguments
    mock_args.return_value = MagicMock(
        user="myuser", password="mypassword", dest="/local/path",
        start="2025-05-01", end="2025-05-10",
        host="host", port=2221, productID="PID0", verbose=True, dry_run=False
    )
    ...
    # 5. Assertions
    # Connection logic
    mock_ssh.connect.assert_called_with("host", port=2221, username="myuser", password="mypassword")
...
def test_main_dry_run_logic(mock_ssh_class, mock_args):
    ...
    mock_args.return_value = MagicMock(
        user="u", password="p", dest="d", start="2025-05-01", end="2025-05-10",
        host="h", port=22, productID="PID0", verbose=False, dry_run=True
    )
    
    mock_ssh = mock_ssh_class.return_value
    mock_sftp = mock_ssh.open_sftp.return_value
    mock_sftp.listdir.side_effect = [["cycle_01"], ["SWOT_L2_LR_SSH_WindWave_01_01_20250505T120000_v1.nc"]]
    
    with patch("s1swotcolocs.download_l2_swot_karin_lr_ssh_windwave_cnes.os.path.exists", return_value=False):
        with patch("s1swotcolocs.download_l2_swot_karin_lr_ssh_windwave_cnes.download_with_progress") as mock_dl:
            swot_script.main()
