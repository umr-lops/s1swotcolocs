...
class TestColocPersistenceAndWrapper(unittest.TestCase):
    ...
    def test_save_netcdf_core(self, mock_mkdir, mock_chmod, mock_to_nc):
        ...
        cdse_output = pd.DataFrame({
            ...
            "geometry": [Polygon([(0,0), (1,1), (1,0)]), Polygon([(0,0), (1,1), (1,0)])],
            "ContentDate": [{"Start": "2025-10-17T15:00:00Z"}, {"Start": "2025-10-17T15:10:00Z"}],
            "id_original_query": ["SWOT file.nc 15:00 15:10", "SWOT file.nc 15:00 15:10"]
        })
    ...
    def test_treat_one_day_wrapper_robustness(self, mock_save, mock_query, mock_geoloc, mock_glob, mock_conf):
        ...
        mock_query.side_effect = [ValueError("CDSE error"), pd.DataFrame({"Name": ["S1"]})]
        ...
        # 5. Execute
        cpt_result = treat_one_day_wrapper("20251017", "/tmp/out", "IW", "fake_conf.yml")
